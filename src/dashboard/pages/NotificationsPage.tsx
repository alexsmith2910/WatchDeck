...
import {
  DeliveryLog,
  type LogFilters,
} from "../components/notifications/DeliveryLog";
import {
  DispatchChart,
  LatencyChart,
...
export default function NotificationsPage() {
  ...
  const loadStats = useCallback(async () => {
    const res = await request<{ data: ApiNotificationStats }>(
      "/notifications/stats",
    );
  ...
  const loadRecentLog = useCallback(async () => {
    ...
    const res = await request<{ data: ApiNotificationLogRow[] }>(
      "/notifications/log?limit=100",
    );
    setRecentLog(res.data?.data ?? []);
  }, [request]);

  const loadMutes = useCallback(async () => {
    const res = await request<{ data: ApiNotificationMute[] }>(
      "/notifications/mutes",
    );
  ...
  const loadEscalations = useCallback(async () => {
    const res = await request<{ data: ApiScheduledEscalation[] }>(
  ...
  const p95ByChannel = useMemo(
    () => latencyP95ByChannel(recentLog),
    [recentLog],
  );
      ...
      <header className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-wd-muted mt-1">
            Delivery health, routing rules, and a full audit trail of every
            alert.
          </p>
        </div>
