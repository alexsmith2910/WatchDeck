...
const colorMap: Record<
  KpiColor,
  { iconBg: string; iconText: string; chartColor: string }
> = {
  primary: {
  ...
  (
    ...
    return (
      <Card
        ...
        onClick={onClick}
      >
        <section className="flex flex-nowrap justify-between h-full">
          <div className="flex flex-col justify-between gap-y-2 p-3 flex-1 min-w-0">
            <div className="flex flex-col gap-y-3">
              ...
              <dd className="flex items-baseline gap-1 text-3xl font-mono font-semibold tracking-tight text-foreground">
                <span>{value}</span>
                {unit && value !== "—" && (
                  <span className="text-sm font-medium text-wd-muted">
                    {unit}
                  </span>
                )}
              </dd>
              ...
              {changeSegments ? (
                <div className="flex items-center gap-x-1.5">
                  {changeSegments.map((seg, i) => (
                    <span key={i} className={changeColorClass[seg.color]}>
                      {seg.text}
                    </span>
                  ))}
                </div>
                ...
                <>
                  {trend && (
                    <Icon
                      ...
                      width={20}
                      className={
                        changeColor ? changeColorClass[changeColor] : ""
                      }
                    />
                  )}
                  {change && (
                  ...
                  {changeLabel && (
                    <span className="text-wd-muted"> {changeLabel}</span>
                  )}
                </>
              )}
            </div>
